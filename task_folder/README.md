...
В первой строке текста "ошибки" описывается тот параметр, который не прошел проверку. Например, ошибка **None is not of type "integer"** - означает, что в требуемом поле указано **None**, вместо необходимого **integer**. Далее указана схема параметров предложенной schema-проверки файлов, где описано правило для данного параметра. В последней строкой текста "ошибки" - **On instance**, указывается какой именно параметр в файле вызвал ошибку проверки. В данном случае вместе требуемого "целого числа" (англ. - integer) в файле указан параметр "None", что означает "ничего". Для исправления нам необходимо в требуемом поле параметра указать целое число, либо переписать правило обработки данного параметра в schema-файле. 

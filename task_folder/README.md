...
В первой строке ошибки описывается тот параметр который не прошел проверку. К примеру ошибка -  *None is not of type "integer"* - указывает, что в требуемом поле указано **None**, вместо требуемого **integer**. Далее описывается схема параметров предложенной schema-проверки файлов, где описано правило для данного параметра. И последней строкой **On instance** указывается какой именно параметр в файле указан ошибочно. В данном случае, вместе требуемого "целого числа" (перевеод с англ. - integer) в файле указан параметр "None", что означает ничего. Для исправления нам требуется в требуемом поле параметра указать целое число. 

 Если вам попался ответ **None** в строке **On instance** и первый параметр описывает ошибку как - **None is not of type "object"** в таких случаях файл проверки пуст или в нем указан один единственный параметр - **Null он же None** 

...
В первой строке текста "ошибки" описывается тот параметр который не прошел проверку. На пример, ошибка -  *None is not of type "integer"* - означает, что в требуемом поле указано **None**, вместо необходимого **integer**. Далее описывается схема параметров предложенной schema-проверки файлов, где описано правило для данного параметра. Последней строкой **On instance** указывается какой именно параметр в файле указан ошибочно. В данном случае, вместе требуемого "целого числа" (англ. - integer) в файле указан параметр "None", что означает "ничего". Для исправления нам необходимо в требуемом поле параметра указать целое число, либо переписать правило обработки данного параметра в файле schema. 
